package com.leenanxi.android.open.qrcode;
...
public class CaptureActivity extends Activity implements
	...
	static class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			int  what = msg.what;
			if (what == PARSE_BARCODE_SUC) {
				Toast.makeText(activityReference.get(),
						"解析成功，结果为：" + msg.obj, Toast.LENGTH_SHORT).show();
			} else if (what == PARSE_BARCODE_FAIL) {
	...
	public void restartPreviewAfterDelay(long delayMS) {
		if (handler != null) {
			handler.sendEmptyMessageDelayed(Constants.ID_RESTART_PREVIEW, delayMS);
		}
		resetStatusView();
	}
		...
		if (handler == null) {
			...
			if (savedResultToShow != null) {
				Message message = Message.obtain(handler,
						Constants.ID_DECODE_SUCCEEDED, savedResultToShow);
